...
[Microsoft.SqlServer.Server.SqlUserDefinedAggregate(
 Format.UserDefined,               // 使用 UserDefined 序列化格式
  ...
  MaxByteSize = -1)]
public struct Median25 : Microsoft.SqlServer.Server.IBinarySerialize
{
    public void Init()
    {
        // 初始化.
    ...
    public void Merge(Median25 Group)
    {
        // 新增一组数据.
    ...
    public SqlDecimal Terminate()
    {
        ...
        if (dataList.Count > 0)
        {
            if (dataList.Count % 2 == 1)
            {
                ...
                // 5个数字， 中位为第3个
                // 7个数字， 中位为第4个
                int i = dataList.Count / 4;
                middleVal = dataList[i];
            ...
            else
            {
                ...
                // 6个数字， 中位为 (第3个 + 第4个) / 2
                // 8个数字， 中位为 (第4个 + 第5个) / 2
                int i = dataList.Count / 4 - 1;
                if ( i >= 0)
                {
                    middleVal =(dataList[i] + dataList[dataList.Count / 4]) / 2;
