...
[Microsoft.SqlServer.Server.SqlUserDefinedAggregate(
 Format.UserDefined,               // 使用 UserDefined 序列化格式
  ...
  MaxByteSize = -1)]
public struct Median75 : Microsoft.SqlServer.Server.IBinarySerialize
{
    public void Init()
    {
        // 初始化.
    ...
    public void Merge(Median75 Group)
    {
        // 新增一组数据.
    ...
    public SqlDecimal Terminate()
    {
        ...
        if (dataList.Count > 0)
        {
            if (dataList.Count % 2 == 1)
            {
                ...
                // 5个数字， 中位为第3个
                // 7个数字， 中位为第4个
                middleVal = dataList[(dataList.Count / 4)*3];
            }
            else
            {
                // 当数量为偶数的时候.
                ...
                // 6个数字， 中位为 (第3个 + 第4个) / 2
                // 8个数字， 中位为 (第4个 + 第5个) / 2
                int i = dataList.Count / 4 * 3 - 1;
                if (i >= 0)
                {
                    middleVal =
                     (dataList[(dataList.Count / 4) * 3 - 1]
                     + dataList[(dataList.Count / 4) * 3]) / 2;
                }
                else
                {
                    middleVal =
